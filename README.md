...
The host half owns the Loader entry, its `Config` schema, and the search provider; the browser half is a single client bundle (`lib/client.js`) that registers the entry's configuration card into the two surfaces dsh reserves for a bundle's own configuration, both over the same `web-search-tavily` namespace. `plugins.bundle.config` carries the form on the bundle's own page, so the settings sit directly under the description — one click from the Plugins list. `plugins.row.config` carries the configure control on the row this bundle's patch inserts, which opens the same form under its own page. Both keys address the **name the profile declares**: the Plugins page dispatches the bundle's declared name for both slots, which is the package's own name unless the profile installs it under an alias. The client therefore reads the plugin manager's bundle list, finds the entry whose row loads `@junjiangao/dsh-web-search-tavily`, and keys both surfaces to the name it reports — falling back to the package's own name when no manager answers. Both render the same form, so a save from either lands in the same place. Nothing registers into `plugins.item`: that slot lists the official host-plane settings pages beside the official bundles, and a card a bundle registers there shows the default glyph whatever its manifest declares, because the slot's artwork map is keyed to the four shipped item ids (`shell`, `agent-loop`, `subagent`, `web-search`). Every field is `volatile()`, so a committed edit updates the `Volatile` refs the provider reads per search — no re-registration and no restart. The cards register only while the Host serves the `web-search-tavily` namespace, so a deployment without the provider shows no trace of them. `apiKey` carries `role('secret')` (redacted on every settings wire) and `apiKeyEnv` carries `role('credential-ref')`, the same declarations the official `web-search-deepseek` provider uses.
...
- **The configuration lives on the bundle, not in the Official group** — `plugins.bundle.config` renders the form on the bundle's own page and `plugins.row.config` gives that row its configure control; neither adds a card to the Official group. Both are keyed by the *declared* bundle name, which the client resolves from the plugin manager's bundle list, so an aliased install (a profile dependency key left over from a package rename) still matches — a package-name key alone would render nothing.
