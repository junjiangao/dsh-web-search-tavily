...
The host half owns the Loader entry, its `Config` schema, and the search provider; the browser half is a single client bundle (`lib/client.js`) that registers the entry's configuration card into two Plugins page surfaces. `plugins.item` carries the card the Official group lists — its id is the `web-search-tavily` namespace the Host serves, which is what lets the page hand that entry's form to the card it opens, so the settings open in one click the way the shipped provider pages do. `plugins.row.config` carries the configure control on the row this bundle's patch inserts, keyed `<package>#<row id>` (`@junjiangao/dsh-web-search-tavily#web-search-tavily`). Both render the same form over the same namespace, so a save from either lands in the same place. Every field is `volatile()`, so a committed edit updates the `Volatile` refs the provider reads per search — no re-registration and no restart. The card registers only while the Host serves the `web-search-tavily` namespace, so a deployment without the provider shows no trace of it. `apiKey` carries `role('secret')` (redacted on every settings wire) and `apiKeyEnv` carries `role('credential-ref')`, the same declarations the official `web-search-deepseek` provider uses.
...
The card also asks the credentials domain about the reference `apiKeyEnv` currently names and states the answer on the form — "key configured: credential TAVILY_API_KEY" or "key not configured, searches run keyless" — so a search that would be unauthenticated says so before it is made. That read is optional: a deployment that exposes no credentials domain to the browser still renders the form, and the status line is simply absent.
