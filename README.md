...
- **The configuration lives on the bundle, not in the Official group** — `plugins.bundle.config` renders the form on the bundle's own page and `plugins.row.config` gives that row its configure control; these are the two surfaces dsh reserves for a bundle's own configuration, and neither adds a card to the Official group.
